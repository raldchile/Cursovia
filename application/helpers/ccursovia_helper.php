...
function FechaToCastle($FechaMsg)
{
    ...
    $consultDate = new DateTime($fecha_consulta);
    $dateMsg = new DateTime($fecha);
    $diff = $dateMsg->diff($consultDate);
    $dias_diff = $diff->days;
    ...
    $anioConsulta = date('Y', strtotime($fecha_consulta));
    ...
    if ($fecha_consulta == $fecha) {
        return substr($FechaMsg, 10, 6);
    } elseif ($dias_diff == 1) {
        return 'Ayer';
    } elseif ($dias_diff > 1 and $dias_diff <= 7) {
        return $nombredia;
    } elseif ($anioConsulta == $anio) {
...
{
    $CI = &get_instance();
    $CI->load->model('cmessages_model');

    $count = $CI->cmessages_model->GetAllUnreadMessages();
